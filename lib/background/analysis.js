...
const ANALYZE_MODE_DEFINITIONS = {
  idea_timeline: {
    label: 'Fikir Evrimi',
    defaultPrompt: 'Kayitlari kronolojik sirada incele. Her donemde altin, gumus, nasdaq ve bist100 gibi piyasalara dair baskin gorusu ozetle. Ilk donem ve son donem arasindaki yon degisimini netlestir.'
  },
  consistency_map: {
    label: 'Tutarlilik Haritasi',
    defaultPrompt: 'Secili kayitlari piyasa bazinda (altin/gumus/nasdaq/bist100 vb.) karsilastir. Tutarli tezleri, celiskili yorumlari ve celiski siddetini cikar. Her celiski icin hangi kanal ne demis kanitla belirt.'
  },
  consensus_view: {
    label: 'Konsensus Gorunumu',
    defaultPrompt: 'Secili kayitlardan toplu gorusu cikar. Her piyasa icin baskin yonu (yukselis/dusus/yatay), bu yone katilan kanal sayisini ve karsi gorusleri yaz. Sonunda genel konsensus ozetini ver.'
  },
  risk_opportunity: {
    label: 'Risk ve Firsat',
    defaultPrompt: 'Kayitlardan cikan risk ve firsatlari kisa/orta vade olarak ayir. Her madde icin olasilik-etki degerlendirmesi ve korunma/aksiyon onerisi ekle. En kritik 5 maddeyi onceliklendir.'
  },
  market_direction_board: {
    label: 'Piyasa Yon Karnesi',
    defaultPrompt: 'Altin, gumus, nasdaq, bist100 ve diger gecen piyasalari tablo mantiginda ozetle. Her piyasa icin mevcut yon, guc derecesi (zayif/orta/guclu), temel gerekce ve belirsizlik notu ver.'
  },
  catalyst_watch: {
    label: 'Tetikleyici Takibi',
    defaultPrompt: 'Kayitlarda gecen tetikleyicileri (faiz karari, enflasyon verisi, jeopolitik gelisme, bilanco vb.) cikar. Her tetikleyici icin hangi piyasayi nasil etkileyebilecegini ve izleme onceligini belirt.'
  },
  custom: {
    label: 'Ozel Mod',
...
const LEGACY_ANALYZE_MODE_ALIASES = {
  argument_shift: 'consensus_view'
};

function normalizeAnalyzeMode(mode){
  const m = String(mode || '').trim().toLowerCase();
  if(Object.hasOwn(LEGACY_ANALYZE_MODE_ALIASES, m)) return LEGACY_ANALYZE_MODE_ALIASES[m];
